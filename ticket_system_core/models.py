...
class User(Base):
    """Tabel Users: Menyimpan data Admin & Customer"""
    __tablename__ = 'users'
...
class Event(Base):
    ...
    # Kolom Baru: Kapasitas Total (untuk referensi generate seat)
    total_capacity = Column(Integer, nullable=False, server_default='0') 

    admin = relationship("User", back_populates="admin_events")
    bookings = relationship("Booking", back_populates="event")
    
    # Relasi ke Seat (One-to-Many) - cascade delete biar bersih kalau event dihapus
...
class Booking(Base):
    ...
    # Relasi: Satu booking punya banyak kursi
...
class Seat(Base):
    """Tabel Baru: Manajemen Kursi (Seat)"""
    __tablename__ = 'seats'

    id = Column(Integer, primary_key=True)
    event_id = Column(Integer, ForeignKey('events.id'), nullable=False)
    
    # Jika null = kosong. Jika terisi = sudah dibooking.
    booking_id = Column(Integer, ForeignKey('bookings.id'), nullable=True)
    
    seat_label = Column(String(10), nullable=False) # Contoh: A1, B2, S001
    ...
    def __repr__(self):
        return f"<Seat {self.seat_label} (Booked: {self.is_booked})>"
